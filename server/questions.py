# Adapted from https://github.com/kartikn27/nlp-question-detection
# TO BE REPLACED
...
class IsQuestion():
    ...
    def __get_question_words_set(self):
        question_word_list = ['what', 'where', 'when', 'how', 'why', 'define', '?']
        return set(question_word_list)

    def __get_verb_word_set(self):
        return set(
            ['did', 'do', 'does', 'have', 'has', 'am', 'is', 'are', 'can', 'could', 'may', 'would', 'will', 'should'
                                                                                                            "didn't",
    ...
    def predict_question(self, text):
        ...
        if bool(self.__get_question_words_set().intersection(words)):
            return 1
        if text.split()[0] in self.__get_verb_word_set():
            return 1
        features = {}
        for word in words:
            features['contains({})'.format(word.lower())] = True
...
if __name__ == "__main__":
    isQ = IsQuestion()
    # Original look at accuracy
    # df_1 = pd.read_csv('queries-10k-txt.csv', sep='\t')
    ...
    print(isQ.predict_question('what is your name'))
    print(isQ.predict_question('is mitosis the same as meiosis'))
    print(isQ.predict_question('This is a statement'))
    print(isQ.predict_question('does google know what you\'re thinking'))
    print(isQ.predict_question('what is mitosis'))
    print(isQ.predict_question('how many planets are there'))
